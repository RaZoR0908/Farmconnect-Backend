...
exports.getAllProducts = async (req, res) => {
  try {
    ...
    const buyerRole = req.user?.role || 'CUSTOMER'; // Default to customer pricing if not logged in
    ...
    // Add pricing info based on buyer role
    const productsWithPricing = data.map(product => {
      const pricing = calculatePrice(product.price, buyerRole);
      return {
        ...product,
        pricing: {
